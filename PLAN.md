...
- **Notification** — per-user event rows for the interactions that actually exist here: **follow, like, reply**, each recorded on both its local and its federated path (R90). Four kinds this line used to name are dropped, each because of a fact about the model rather than a scope cut: *review on your film* — no such concept; `Film` is one global row that many users review, so "your film" has no referent (R91). *Follow request* — follows are accepted unconditionally, so there is no pending state to notify about. *Mention* — there is no mention parser, and `Hashtag` is unbuilt too. *Import/export job completion* — import and export run in-request, and the only async work in the app (the TMDB backfill) records no one who queued it.
