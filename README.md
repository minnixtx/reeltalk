...
- [ ] Polish & first public instance (M6) — **in progress**: daily `pg_dump` backup job ✅, the getting-started page (`/welcome/`, which settles the old "guided tour" item) ✅, artwork and the visual identity above ✅, with an owner-led polish pass refining it item by item. The real-domain target is settled and pre-flight-checked; deploy-readiness work has started — the forwarded TLS scheme is believed only from the operator's own proxy, every published URL and every inbound signature check now reads that single trusted scheme instead of the header directly, session cookies carry `Secure` according to the transport they actually arrived on, every sign-up call to action now follows the invite-only policy rather than advertising an account you cannot get, and the backup path has a real restore half: a `restore_database` command that must be told which database to target and refuses the live one without `--force`, drilled end to end against a throwaway database and reproduced in the test suite. [DEPLOYING.md](DEPLOYING.md) documents the whole procedure. Remaining: the cutover itself.
