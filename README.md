...
- [ ] Polish & first public instance (M6) — **in progress**: daily `pg_dump` backup job ✅, the getting-started page (`/welcome/`, which settles the old "guided tour" item) ✅, artwork and the visual identity above ✅, with an owner-led polish pass refining it item by item. The real-domain target is settled and pre-flight-checked; deploy-readiness work has started — the forwarded TLS scheme is believed only from the operator's own proxy, every published URL and every inbound signature check now reads that single trusted scheme instead of the header directly, and session cookies carry `Secure` according to the transport they actually arrived on. Remaining: close the plain-HTTP exposure, make the sign-up calls follow the invite-only policy, rehearse a backup restore, and cut over.
