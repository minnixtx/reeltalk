...
| M1 — core film domain (first working version, part 1) | 🔄 In progress — increment 3 of ~7 done: core app + Film domain (`cbf72c2`), social app + custom User + auth + setup wizard (`a41c87b`), Shelf/ShelfFilm + binary default shelves + merge/absorb (`d45a1e4`, `cac1627`). **Next session starts at increment 4: Status/Review/ReviewRating + watch rules.** Plan for the increments is in §2 below. |
...
- **R16 — Merge/absorb semantics.** `Film.merge_into(canonical)`, all-or-nothing: (1) backfill only empty fields onto the canonical via an explicit field list — subtitle/description/year/runtime/poster/tmdb_id/imdb_id scalars plus order-preserving unions for genres/directors/cast; identity/provenance (title/sort_title/origin_id/remote_id) and dates are never touched; (2) re-point related rows — ShelfFilm (shelved_date + actor preserved, duplicate-on-same-shelf deduped) and `User.blocked_films` — one block per model in `_repoint_related()`, so Status gets its block when it lands in increment 4; (3) record the MergedFilm old→new mapping; (4) delete the absorbed row; the canonical is saved **last** because a backfilled unique `tmdb_id` would otherwise collide with the value the absorbed row still holds. Owner may veto.
